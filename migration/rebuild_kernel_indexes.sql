...
||
|| Revision 1.2  2010-11-29 16:00:26  shmyg
|| Refresh 29.11.2010
||
...
SPOOL run_rebuild_kernel_indexes.sql

SELECT	'ALTER INDEX SYSADM.' || index_name || ' REBUILD TABLESPACE ' || new_ts_name || ' NOLOGGING;'
FROM	kernel_indexes
WHERE	version_date =
	(
...
AND	deleted IS NULL
AND	new_ts_name != 'I_BSCS_TINY'
/

SPOOL OFF

@run_rebuild_kernel_indexes.sql
!rm run_rebuild_kernel_indexes.sql
